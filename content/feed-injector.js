...
(() => {
  ...
  function ensureFeedStyles() {
    ...
    style.textContent = `
      ...
      /* Hover or active state reveals button */
  ...
  function scanYouTubeCards() {
    const cards = document.querySelectorAll('ytd-rich-item-renderer, ytd-video-renderer, ytd-grid-video-renderer, ytd-compact-video-renderer, ytd-reel-item-renderer');
    cards.forEach((card) => {
      if (card.querySelector('.sparksub-feed-btn')) return;
      const link = card.querySelector('a#thumbnail[href*="/watch?v="], a[href*="/watch?v="], a[href*="/shorts/"]');
      if (!link) return;
      ...
      const coverContainer = card.querySelector('#thumbnail, ytd-thumbnail, .ytd-thumbnail') || link;
      const titleElem = card.querySelector('#video-title, #video-title-link');
      const title = titleElem?.getAttribute('title') || titleElem?.textContent?.trim() || `YouTube 视频 (${videoId})`;
      const authorElem = card.querySelector('ytd-channel-name, #channel-name, #text.ytd-channel-name');
